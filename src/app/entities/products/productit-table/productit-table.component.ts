...
import { ProductitTable }  from '../product.model';
 ...
 */
@Component({
  selector: 'productit-table',
  templateUrl: './productit-table.component.html',
  styleUrls:  ['./productit-table.component.scss'],
  providers: [GenericDialogComponent]
})
export class ProductitTableComponent implements OnInit, OnChanges {
  @Input() public displayedColumns = ['select', 'code', 'ptype', 'slug', "vendorname","ume", "pu"];
  ...
  private table_columns = ["select", 'code','ptype', "slug", "vendorname","ume", 'fume', 'moneda' , "pu"];
  ...
  private dataRecordsSource: BehaviorSubject<ProductitTable[]>;
  ...
  public selection = new SelectionModel<ProductitTable>(true, []);

  constructor(private productCtrl: ProductController,
              public dialogService: MatDialog
    ){
    this.dataRecordsSource = this.productCtrl.productsDataSource;
  }

  ngOnInit(){
    this.dataSource = new ProductDataSource(this.dataRecordsSource, this.paginator, this.sort)
    this.productCtrl.selectionModel = this.selection;
...
export class ProductDataSource extends DataSource<any> {
  ...
  constructor(private sourceData: BehaviorSubject<ProductitTable[]>,
  ...
  connect(): Observable<ProductitTable[]> {
  ...
  getSortedData(): ProductitTable[]{
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        ...
        case 'ptype':      [propertyA, propertyB] = [a.ptype, b.ptype]; break;
